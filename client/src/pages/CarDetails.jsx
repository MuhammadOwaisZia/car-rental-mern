...
const CarDetails = () => {
  ...
  const fetchCar = async () => {
    try {
      const { data } = await axios.get(`${backendUrl}/api/user/cars/${id}`)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        `${backendUrl}/api/user/bookings`,
        { carId: id, startDate: pickupDate, endDate: returnDate },
        ...
        <form onSubmit={handleSubmit} className='shadow-lg h-max sticky top-18 rounded-xl p-6 space-y-6 text-gray-500'>
          ...
          <div className='flex flex-col gap-2'>
            ...
            <input
              ...
              className='border border-borderColor px-3 py-2 rounded-lg outline-none focus:ring-2 focus:ring-primary'
              required
            />
          </div>
